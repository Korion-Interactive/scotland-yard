...
public class PlayerTurnSystem : BaseSystem<GameEvents, PlayerTurnSystem>
{
    ...
    private void GameStart(BaseArgs args)
    {
        ...
        {
            PauseAndBlur(true);
            PopupManager.ShowPrompt("pass_device", "mr_x_turn_starts", (o) => { PauseAndBlur(false); firstTurn(); }, GameState.Instance.MrX.PlayerDisplayName);
        }
        else
        {
            firstTurn();
    ...
    private void PlayerHasMoved(MoveArgs move)
    {
        ...
        {
            PauseAndBlur(true);
            PopupManager.ShowPrompt("pass_device", "mr_x_turn_starts", (o) => { PauseAndBlur(false); nextPlayer(); }, GameState.Instance.MrX.PlayerDisplayName);
        }
        else
        {
            nextPlayer();
