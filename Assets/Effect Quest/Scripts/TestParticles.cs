...
public class TestParticles : MonoBehaviour
{
	...
	void OnGUI ()
	{
		...
		// Show version number
		GUI.Window(1, new Rect((Screen.width-260), 5, 250, 80), AppNameWindow, "Effect Quest 0.2.5");
		...
		GUI.Window(3, new Rect((Screen.width-300), Screen.height-85, 290, 80), ParticleInformationWindow, "Information");
	}
	...
	void ParticleInformationWindow(int id)
	{
		if(m_CurrentParticleIndex>=0)
		{
			GUI.Label(new Rect(12, 25, 300, 20), "Up/Down: Type "+(m_CurrentCategoryIndex+1)+" of "+m_CategoryList.Length+" "+m_CurrentCategoryName);
			GUI.Label(new Rect(12, 50, 300, 20), "Left/Right: Particle "+(m_CurrentParticleIndex+1) + " of " + m_CurrentCategoryChildCount +" "+ m_CurrentParticleName);
		}
	}
