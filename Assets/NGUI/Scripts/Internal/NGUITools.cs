	...
	static public void UpdateWidgetCollider (BoxCollider box, bool considerInactive)
	{
		if (box != null)
		{
			...
			if (w != null)
			{
				...
				box.center = new Vector3((region.x + region.z) * 0.5f, (region.y + region.w) * 0.5f, box.center.z);
				box.size = new Vector3(region.z - region.x, region.w - region.y, box.size.z); // PATCH: z value added
			}
			else
			{
				Bounds b = NGUIMath.CalculateRelativeWidgetBounds(go.transform, considerInactive);
				box.center = b.center;
                box.size = box.size;//new Vector3(b.size.x, b.size.y, b.size.z);
			}
#if UNITY_EDITOR
