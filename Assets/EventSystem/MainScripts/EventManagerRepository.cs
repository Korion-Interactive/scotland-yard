...
public sealed class EventManagerRepository
{
    ...
    //For retrieving event managers
    public SystemEventManager<TEventType> GetSingleInstanceSystemEventManager<TEventType>() where TEventType : struct, IComparable, IConvertible, IFormattable
    {
        EventManager eventManager;
        ...
        if (eventManager == null)
        {
            SystemEventManager<TEventType> tempManager = new SystemEventManager<TEventType>();
            if (tempManager != null) //In case construction throws an error
                eventManagers.Add(typeof(TEventType), tempManager);
            return tempManager;
        }
        else
            return (SystemEventManager<TEventType>)eventManager;
    }
