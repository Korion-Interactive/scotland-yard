...
#if LOG_ALL_MESSAGES || LOG_BROADCAST_MESSAGE
        Debug.Log("MESSENGER\t" + System.DateTime.Now.ToString("hh:mm:ss.fff") + "\t\t\tInvoking \t\"" + eventName + "\"");
        //this.LogInfo(string.Format("\t{0} \t<color=#777700> {1} </color> \t<color=#5555AA> {2} </color> \t<color=#337733> {3} </color>", System.DateTime.Now.ToString("hh:mm:ss.fff"), eventName, arg1, arg2));
		#endif
		#if REQUIRE_LISTENER
        OnBroadcasting(eventID);
		#endif
