...
[System.Serializable]
[UnityEditor.CustomEditor(typeof(ColorCorrectionLut))]
public class ColorCorrectionLutEditor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        Rect r = default(Rect);
        this.serObj.Update();
        EditorGUILayout.LabelField("Converts textures into color lookup volumes (for grading)", EditorStyles.miniLabel, new GUILayoutOption[] {});
        //EditorGUILayout.LabelField("Change Lookup Texture (LUT):");
        //EditorGUILayout.BeginHorizontal ();
        //var r : Rect = GUILayoutUtility.GetAspectRect(1.0f);
        Texture2D t = null;
        //EditorGUILayout.Space();
        this.tempClutTex2D = EditorGUILayout.ObjectField(" Based on", this.tempClutTex2D, typeof(Texture2D), false, new GUILayoutOption[] {}) as Texture2D;
        ...
        if (tex && ((this.target as ColorCorrectionLut).basedOnTempTex != AssetDatabase.GetAssetPath(tex)))
        {
            ...
            {
                if (GUILayout.Button("Convert and Apply", new GUILayoutOption[] {}))
                {
