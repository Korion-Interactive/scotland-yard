...
namespace FullInspector.Modules.SerializerMigrationUtility {
    public class fiSerializerMigrationUtilityEditorWindow : EditorWindow {
        ...
        public static void ShowWindow() {
            var window = EditorWindow.GetWindow<fiSerializerMigrationUtilityEditorWindow>(/*utility:*/ true);

            window.minSize = new Vector2(1, 1);
            window.position = new Rect(window.position.x + 20, window.position.y + 20, 883, 600);

			window.titleContent.text = "Serializer Migration Utility";
        }
        ...
        private void BeforeMigrate() {
            if (fiEditorSerializationManager.DisableAutomaticSerialization == false) {
                ...
                fiEditorSerializationManager.DisableAutomaticSerialization = true;
            }
        }
		...
		// disabled deprecation warning concerning EditorApplication.OpenScene/currentScene to avoid falsely refactoring FullInspector2
