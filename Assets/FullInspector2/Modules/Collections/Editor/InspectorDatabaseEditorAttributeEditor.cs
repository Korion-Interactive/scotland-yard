...
namespace FullInspector.Modules.Collections {
    ...
    public class InspectorDatabaseEditorAttributeEditor<TDerived, T> :
        ...
        private static void EnsureInstance(ref IList<T> element) {
            if (element == null) {
                element = (TDerived)InspectedType.Get(typeof(TDerived)).CreateInstance();
            }
        }
        ...
        /// list.
        /// </summary>
        private static void ChangeEditedElement(ref IList<T> list, int offset) {
            var metadata = fiGlobalMetadata.Get<InspectorDatabaseEditorMetadata>(list);
            metadata.CurrentIndex += offset;

            if (metadata.CurrentIndex >= list.Count) {
                ListHelpers.Add(ref list);
                metadata.CurrentIndex = list.Count - 1;
            }
        }
        ...
        /// </summary>
        private static void RemoveEditedElement(ref IList<T> list) {
            if (CanDelete(list)) {
                ListHelpers.RemoveAt(ref list, CurrentIndex(list));
            }
        }
        ...
        private Rect DrawHeader(Rect region, GUIContent label, ref IList<T> list) {
            ...
            if (GUI.Button(backRect, "<<")) {
                ChangeEditedElement(ref list, -1);
            }
            EditorGUI.EndDisabledGroup();
        ...
        protected override IList<T> Edit(Rect region, GUIContent label, IList<T> list, InspectorDatabaseEditorAttribute attribute, fiGraphMetadata metadata) {
            ...
            region = DrawHeader(region, label, ref list);
        ...
        private static class ListHelpers {
            public static void Add<T>(ref IList<T> list) {
                if (list.GetType().IsArray) {
                    T[] arr = (T[])list;
                    ArrayUtility.Add(ref arr, default(T));
                    list = arr;
                }
                else {
                    list.Add(default(T));
            ...
            public static void RemoveAt<T>(ref IList<T> list, int index) {
                if (list.GetType().IsArray) {
                    T[] arr = (T[])list;
