...
namespace FullInspector.Internal {
    ...
    public abstract class BaseCollectionPropertyEditor<TActual, TCollection, TItem, TAddItem> : PropertyEditor<TCollection>
        ...
        public class PageMetadata : IGraphMetadataItem {
        ...
        private readonly float PagedRegionVerticalMargin = EditorGUIUtility.standardVerticalSpacing * 1.3f;
        ...
        public void DoEdit(Rect region, GUIContent label, ref TCollection collection, fiGraphMetadata metadata,
            ...
            Rect addItemButtonRect, addItemItemRect;
            {
                ...
                RectTools.SplitLeftHorizontalExact(baseAddItemRect,
        ...
        public override TCollection Edit(Rect region, GUIContent label, TCollection collection, fiGraphMetadata metadata) {
            ...
            if (ShouldDisplayPageControls(collection)) {
                ...
                const float margin_DecButton_IncButton = 5;
                ...
                RectTools.SplitHorizontalFlexibleMiddle(pagedRect, regionIndexSelectorWidth, regionIncDecWidth,
                    out rect_RegionIndexSelector, out rectDummy, out rect_RegionIncDec);


                RectTools.CenterRect(rect_RegionIndexSelector, EditorGUIUtility.singleLineHeight,
                ...
                if (EditorGUI.EndChangeCheck()) {
                    ...
                    if (newEndIndex <= pageMetadata.PageStartIndex) newEndIndex = pageMetadata.PageStartIndex + 1;
                    EnsureWithinLimits(ref newEndIndex, 0, collection.Count);
                ...
                if (GUI.Button(rect_DecButton, "<<")) {
                    ShiftPageBackward(pageMetadata, pageMetadata.PageEndIndex - pageMetadata.PageStartIndex);
                }
                EditorGUI.EndDisabledGroup();

                EditorGUI.BeginDisabledGroup(!(pageMetadata.PageEndIndex < collection.Count - 1));
                if (GUI.Button(rect_IncButton, ">>")) {
                    ShiftPageForward(pageMetadata, pageMetadata.PageEndIndex - pageMetadata.PageStartIndex, collection.Count);
                }
                EditorGUI.EndDisabledGroup();
