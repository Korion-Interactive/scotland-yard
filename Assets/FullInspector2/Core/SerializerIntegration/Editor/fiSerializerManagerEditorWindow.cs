...
namespace FullInspector.Internal {
    ...
    public class fiSerializerManagerEditorWindow : EditorWindow {
        ...
        public static void ShowWindow() {
            EditorWindow window = EditorWindow.GetWindow<fiSerializerManagerEditorWindow>();

            // try to center it in the middle of the screen, but it seems like Screen.width and
            // Screen.height sometimes give strange values...
            float width = 880;
            float height = 390;
            float x = (Screen.width - width) / 2;
            float y = (Screen.height - height) / 2;
            if (window.position.x > x) x = window.position.x;
            if (window.position.y > y) y = window.position.y;

            window.position = new Rect(x, y, width, height);

			window.titleContent.text = "Serializer Manager";
        }
