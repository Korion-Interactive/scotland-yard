    ...
    internal class AbstractTypeInstanceOptionManager {
        ...
        private static DisplayedType[] GetDisplayedTypes(Type baseType) {
            ...
            if (TypeCache.TryGetValue(baseType, out result) == false) {
                ...
                foreach (var type in fiReflectionUtilitity.GetCreatableTypesDeriving(baseType)) {
                    string typeName = "";
                    if (type.HasAttribute<InspectorDropdownNameAttribute>()) {
                        typeName = type.GetAttribute<InspectorDropdownNameAttribute>().DisplayName;
                    }
