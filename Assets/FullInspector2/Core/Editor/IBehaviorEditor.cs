...
namespace FullInspector {
    ...
    public abstract class BehaviorEditor<TBehavior> : IBehaviorEditor
        ...
        public void Edit(Rect rect, UnityObject behavior) {
            // TODO: run foldouts properly with hiearchyMode = false
            // set hierachy mode to true otherwise foldouts
            // are indented too far
            var hiearchyMode = EditorGUIUtility.hierarchyMode;
            ...
            EditorGUIUtility.hierarchyMode = hiearchyMode;
        }
        ...
        private static void CheckForPrefabChanges(UnityObject target) {
            ...
            // point.
            bool hadPriorData;
            if (ObjectModificationDetector.WasModified(obj, out hadPriorData) &&
                ...
                //
                // We want to make sure we do this when we actually have serialized data so inline
                // object constructors work, ie, we don't overwrite the user's constructor.
                if (hadPriorData &&
                    obj.SerializedStateKeys.Count == 0 && obj.SerializedStateValues.Count == 0) {

                    // When Unity resets, it only clears out values that are in its serialized
                    // model. We need to also clear out values that it does not serialize.
                    obj.SaveState();
                    ...
                    // run reset again to get the proper data
                    fiRuntimeReflectionUtility.InvokeMethod(target.GetType(), "Reset", target, null);
                    obj.SaveState();
