...
public class MakePreprocessedPluginSource : IPreprocessBuildWithReport
{
      public int callbackOrder => 0;
        ...
        // could be made more efficient ... perhaps just returning the sdk string rather than the int
        private static int SDKVersion()
        {
            int version = 0;
            string sdkPath = System.Environment.GetEnvironmentVariable("SCE_ORBIS_SDK_DIR");
            if (sdkPath != null)
            {
                ...
                if (File.Exists(versionFile))
                {
                    ...
                    {
                        if (line.Contains("SCE_ORBIS_SDK_VERSION"))
                        {
                            if (line.Contains("#define"))
                            {
                                Console.WriteLine("[ps4] Found SDK version: " + versionFile + ": " + line);
                                string[] words = line.Split(new char[] { ' ' }, StringSplitOptions.RemoveEmptyEntries);
                                Console.WriteLine("[ps4] words {0} {1} {2}", words[0], words[1], words[2]);
                                if (words[2].StartsWith("(0x"))
                                {
                                    string verstring = words[2].Substring(3, 8);
                                    version = Convert.ToInt32(verstring, 16);
                                    Console.WriteLine("[ps4] sdk version data {0} {1} ", verstring, version);
                                    break;
                                }
                            }
                        }
      ...
      public void OnPreprocessBuild(BuildReport report)
      {
            ...
            string packagename="com.unity.savedata.ps4";
            ...
            string preprocessedSourcePath = Path.GetFullPath($"{fi.Directory}/../Plugins/PS4/Source/{packagename}_preprocessed.cpp");
            ...
            var sdkloc = Environment.GetEnvironmentVariable("SCE_ORBIS_SDK_DIR");
            processInfo = new ProcessStartInfo($"{sdkloc}/host_tools/bin/orbis-clang.exe", Command);
