...
namespace SaveData
{
	...
	static char sIconBuffer[SCE_SAVE_DATA_ICON_FILE_MAXSIZE];
	...
	{
		memset(sIconBuffer, 0xAA, SCE_SAVE_DATA_ICON_FILE_MAXSIZE);

		memset(&icon, 0x00, sizeof(icon));
		icon.buf = sIconBuffer;
		icon.bufSize = SCE_SAVE_DATA_ICON_FILE_MAXSIZE;
	}
