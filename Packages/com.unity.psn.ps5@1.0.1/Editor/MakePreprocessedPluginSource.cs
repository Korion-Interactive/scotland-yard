...
#if UNITY_PS4
    static string sdkEnvVar = "SCE_ORBIS_SDK_DIR";
    static string sdkVersionString = "SCE_ORBIS_SDK_VERSION";
    static string sdkClangExe = "orbis-clang.exe";
    static string sdkPlatform = "PS4";
    static string sdkPlatformL = "ps4";
#else
    static string sdkEnvVar = "SCE_PROSPERO_SDK_DIR";
    static string sdkVersionString = "SCE_PROSPERO_SDK_VERSION";
    static string sdkClangExe = "prospero-clang.exe";
    static string sdkPlatform = "PS5";
    static string sdkPlatformL = "ps5";
#endif
        ...
        {
            int version = 0;
            string sdkPath = System.Environment.GetEnvironmentVariable(sdkEnvVar);
            if (sdkPath != null)
            {
                ...
                if (File.Exists(versionFile))
                {
                    ...
                    {
                        if (line.Contains(sdkVersionString))
                        {
      ...
      public void OnPreprocessBuild(BuildReport report)
      {
            ...
            string preprocessedSourcePath = Path.GetFullPath($"{fi.Directory}/../Plugins/{sdkPlatform}/Source/com.unity.psn.{sdkPlatformL}_preprocessed.cpp");
            ...
            if (File.Exists(preprocessedSourcePath))
            {
                ...
                {
                        System.Console.WriteLine($"checking byte {i}");
            ...
            string rttiOption = "-frtti";
#if UNITY_2020 || UNITY_2021
            rttiOption = "";
#endif

            File.SetAttributes(preprocessedSourcePath, FileAttributes.Normal);
            string IncludePath = Path.GetFullPath($"{fi.Directory}/PSNCommon/SharedInclude");
            string Command = $"-E \"{fullpath}\" {rttiOption} -I\"{IncludePath}\" -o \"{preprocessedSourcePath}\"";
            var sdkloc = Environment.GetEnvironmentVariable(sdkEnvVar);
            processInfo = new ProcessStartInfo($"{sdkloc}/host_tools/bin/{sdkClangExe}", Command);
...
#if UNITY_2020 || UNITY_2021
public class PSNPS5ProjectPostProcess : UnityEditor.PS5.IPostProcessPS5
{
