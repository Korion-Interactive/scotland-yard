...
#define WaveReadLaneAt WaveReadLaneShuffle
...
#define DEFINE_SHUFFLE_TYPE(TYPE) \
TYPE WaveReadLaneShuffle(TYPE v, uint laneIndex) \
{ \
    bool isValid = false; \
    do \
    { \
        uint firstLaneIndex = WaveReadLaneFirst(laneIndex); \
        if (laneIndex == firstLaneIndex) \
        { \
            v = ReadLane(v, firstLaneIndex); \
            isValid = true; \
        } \
    } while (!isValid); \
    return v; \
...
DEFINE_SHUFFLE_TYPE(uint);
DEFINE_SHUFFLE_TYPE(int);
DEFINE_SHUFFLE_TYPE(float);
...
#include "Packages/com.unity.render-pipelines.ps4/ShaderLibrary/Compiler/ShaderCompilerOptions.hlsl"
